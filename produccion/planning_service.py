...
def calculate_timeline(maquina, tasks, start_date=None, task_min_start_times=None, task_force_start_times=None, non_working_days=None, half_day_holidays=None, plan_mode='auto'):
    ...
    for task_debug in tasks:
        op_num = task_debug.get('Idorden', '???') if isinstance(task_debug, dict) else getattr(task_debug, 'Idorden', '???')
        
        nivel_debug = 0.0
        keys_to_check = ['Nivel_Planificacion', 'nivel_planificacion', 'nivel_manual', 'Nivel']
        for key in keys_to_check:
            val = task_debug.get(key) if isinstance(task_debug, dict) else getattr(task_debug, key, None)
            if val is not None:
                try:
                    f_val = float(val)
                    if f_val != 0:
                    ...
                    pass
        print(f"TIMELINE OP: {op_num} - Nivel Extraído: {nivel_debug}")
