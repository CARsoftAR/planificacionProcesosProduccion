...
def planificacion_list(request):
    ...
    try:
        ...
        def obtener_clave_ordenamiento(op):
            ...
            return (p_proy_val, p_art_val, -n_plan_val, op.get('Idorden', 9999999), op.get('secuencia_proceso', 999), op.get('OrdenSecuencia', 999999), op.get('OrdenVisual', 999999.0))
...
def planificacion_visual_OLD(request):
    ...
    if start_simulation.date() == _date.today():
        # Hoy: tomar la hora actual, mínimo 07:00
            ...
            for mst, group in groups.items():
                ...
                for i, r in enumerate(group):
                    ...
                    if oid in op_to_nivel:
                        ...
                        # Usar el valor nativo del ERP (columna Nivel de TMAN002)
                        # en lugar de calcular un índice secuencial (i+1) que rompe la jerarquía real.
                        erp_val = r.get('Nivel_Planificacion')
                        if erp_val is not None and str(erp_val).strip() not in ('', '0'):
                            r['Nivel_Planificacion'] = int(float(erp_val))
                        else:
                            r['Nivel_Planificacion'] = 0
                        print(f"DEBUG ERP (Nativo): Proceso='{r.get('Proceso')}' | Articulo='{r.get('Articulo')}' | Nivel={r.get('Nivel_Planificacion')}")
